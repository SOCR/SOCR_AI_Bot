...
```{r warning=F, message=F, error=F, fig.width=12, fig.height=10}
library(plotly)
...
```{r eval=TRUE, warning=F, message=F, error=F}
...
```{r setup, eval=TRUE, warning=F, message=F, error=F}
#### Initial Stuff ####
# clean up
rm(list=ls())
...
```{r manualRidge, fig.align='center', fig.height=5.8, fig.width=8.5, warning=F, message=F, error=F}
# 1. Define RIDGE Loss
...
```{r manualLASSO, fig.align='center', fig.height=5.8, fig.width=8.5, warning=F, message=F, error=F}
...
```{r manualLASSOBackUp, echo=FALSE, eval=FALSE, fig.align='center', fig.height=5.8, fig.width=8.5, warning=F, message=F, error=F}
# # Define a soft-thresholding utility function S(a=beta, b=lambda)
...
```{r LASSOpath, eval=TRUE, fig.align='center', fig.height=5.8, fig.width=8.5, warning=F, message=F, error=F}
...
```{r ridgepath, eval=TRUE, fig.align='center', fig.height=5.8, fig.width=8.5, warning=F, message=F, error=F}
### Plot Solution Path ###
# Ridge
...
```{r fig.height=7,fig.width=7,fig.cap="SSE Contour and Penalty Region (Ridge).", warning=F, message=F, error=F}
plot(p1)
```

```{r fig.height=7,fig.width=7,fig.cap="SSE Contour and Penalty Region (LASSO).", warning=F, message=F, error=F}
plot(p6)
```
...
```{r fig.height=14,fig.width=13,fig.cap="SSE Contour and Penalty Region for 6 values of Alpha.", warning=F, message=F, error=F}
library("gridExtra")
...
```{r LASSOcv, eval=TRUE, fig.align='center', fig.height=5.8, fig.width=8.5, warning=F, message=F, error=F}
plotCV.glmnet <- function(cv.glmnet.object, name="") {
  df <- as.data.frame(cbind(x=log(cv.glmnet.object$lambda), y=cv.glmnet.object$cvm, 
                           errorBar=cv.glmnet.object$cvsd), nzero=cv.glmnet.object$nzero)
...
```{r ridgecv, eval=TRUE, fig.align='center', fig.show='show', warning=F, message=F, error=F}
plotCV.glmnet <- function(cv.glmnet.object, name="") {
  df <- as.data.frame(cbind(x=log(cv.glmnet.object$lambda), y=cv.glmnet.object$cvm, 
                           errorBar=cv.glmnet.object$cvsd), nzero=cv.glmnet.object$nzero)
...
```{r testerror, eval=TRUE, warning=F, message=F, error=F}
...
```{r regplot, eval=TRUE, fig.align='center', fig.height=5.3, fig.width=8.5, warning=F, message=F, error=F}
...
```{r msetab, eval=TRUE, results='asis', warning=F, message=F, error=F}
# Test Set MSE Table 
# create table as data frame
...
```{r echo=F, fig.width=10, fig.height=7, warning=F, message=F, error=F}
