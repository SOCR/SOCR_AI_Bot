...
# virtualenv_install("pyDev", packages = c('llama_index','llama-index-embeddings-huggingface',
#                                          'llama-index-embeddings-langchain','llama-index-llms-huggingface',
#                                          'llama-index-llms-openai','llama-index-embeddings-instructor'))
...
socrragGPT <-function(prompt) {
  req <- POST("http://127.0.0.1:5000/generate_response",
              body = list(prompt = prompt),
              encode = "json")
  
  if (status_code(req) == 200) {
    response_data <- content(req, "parsed")
...
server <- function(input, output, session) {
  ...
  observeEvent(input$submit, {
    ...
    if (nchar(trimws(input$prompt)) > 0) {
      ...
      chatGPT <- py_libs$generate_response(input$apiKey, input$model,  input$prompt)
      print(chatGPT)
